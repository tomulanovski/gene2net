...
#SBATCH --array=0-20
#SBATCH --output=/groups/itay_mayrose/tomulanovski/gene2net/simulations/logs/notung_simulated_%a.out
#SBATCH --error=/groups/itay_mayrose/tomulanovski/gene2net/simulations/logs/notung_simulated_%a.err
...
GENE_TREES_DIR="${BASE_DIR}/${NETWORK}/data/low_dup_ne_100000_height_1_million_trees/1"
GENE_TREES_NHX_DIR="${BASE_DIR}/${NETWORK}/data/low_dup_ne_100000_height_1_million_trees/gene_trees_nhx"
SPECIES_TREE_NEX="${BASE_DIR}/${NETWORK}/species_tree_1_million.nex"
...
if [ ! -f "$SPECIES_TREE_NEX" ]; then
    echo "ERROR: Species tree NEXUS file not found: $SPECIES_TREE_NEX"
    exit 1
fi
...
echo "Species tree NEXUS: $SPECIES_TREE_NEX"
echo "Output directory: $OUTPUT_DIR"

# Step 1: Convert NEXUS to Newick
echo ""
echo "Step 1: Converting NEXUS to Newick..."

grep -E "^\s*tree\s+" "$SPECIES_TREE_NEX" | sed 's/.*= *//' | sed 's/;$//' > "$SPECIES_TREE_NEWICK"

if [ ! -f "$SPECIES_TREE_NEWICK" ] || [ ! -s "$SPECIES_TREE_NEWICK" ]; then
    echo "ERROR: Failed to convert NEXUS to Newick"
    exit 1
fi

echo "Converted species tree saved to: $SPECIES_TREE_NEWICK"
