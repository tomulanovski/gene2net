#!/bin/bash
#SBATCH --job-name=grampa_sims
#SBATCH --array=1-21
#SBATCH --output=/groups/itay_mayrose/tomulanovski/gene2net/simulations/logs/grampa_run_rep1_ILS_low_dup_low_job_%a.out
#SBATCH --error=/groups/itay_mayrose/tomulanovski/gene2net/simulations/logs/grampa_run_rep1_ILS_low_dup_low_job_%a.err
...
OUTPUT_DIR="${BASE_DIR}/${network}/results/ILS_low_dup_low/grampa"
...
if [ $? -eq 0 ]; then
    echo ""
    echo "=========================================="
    echo "GRAMPA completed successfully for ${network}"
