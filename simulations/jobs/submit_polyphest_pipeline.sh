...
# ============================================================================
# POLYPHEST PIPELINE HELPER
# ============================================================================
# This script provides easy submission of the Polyphest pipeline.
#
# Usage:
#   ./submit_polyphest_pipeline.sh <config_name> [options]
#
# Examples:
#   # Run both prep and polyphest for a configuration
#   ./submit_polyphest_pipeline.sh conf_ils_low_10M
#
#   # Run only prep step
#   ./submit_polyphest_pipeline.sh conf_ils_low_10M --prep-only
#
#   # Run only polyphest step (assumes prep already done)
#   ./submit_polyphest_pipeline.sh conf_ils_low_10M --run-only
#
#   # With custom number of replicates
#   ./submit_polyphest_pipeline.sh conf_ils_low_10M --replicates 3
#
#   # Dry run (show commands without executing)
#   ./submit_polyphest_pipeline.sh conf_ils_low_10M --dry-run
# ============================================================================
...
PREP_SCRIPT="${SCRIPT_DIR}/prep_polyphest.sh"
RUN_SCRIPT="${SCRIPT_DIR}/run_polyphest.sh"
...
Options:
  ...
  --run-only        Only run Polyphest (assumes prep is done)
  --percentile N    Polyphest percentile parameter (default: 60)
  --iso-threshold N Isomorphic threshold (default: 0.2)
  --dry-run         Show commands without executing
...
echo "============================================================================"
echo "POLYPHEST PIPELINE"
echo "============================================================================"
echo "Configuration: ${CONFIG}"
echo "Replicates: ${NUM_REPLICATES}"
echo "Networks: ${NUM_NETWORKS}"
echo "Total Polyphest jobs: ${ARRAY_SIZE}"
echo ""
echo "Polyphest parameters:"
...
if [ "$RUN_ONLY" = true ]; then
    ...
    echo "  [x] Run Polyphest"
elif [ "$PREP_ONLY" = true ]; then
    echo "  [x] Prep"
    echo "  [ ] Run Polyphest (skipped - --prep-only)"
else
    echo "  [x] Prep"
    echo "  [x] Run Polyphest (with dependency)"
fi
echo ""
...
if [ "$RUN_ONLY" = false ]; then
    ...
    PREP_CMD="sbatch --export=CONFIG=${CONFIG},NUM_REPLICATES=${NUM_REPLICATES} \
        --output=${LOG_DIR}/prep_polyphest_${CONFIG}_%A_%a.out \
        --error=${LOG_DIR}/prep_polyphest_${CONFIG}_%A_%a.err \
        ${PREP_SCRIPT}"
...
if [ "$PREP_ONLY" = false ]; then
    echo "Submitting Polyphest run job..."
    
    EXPORT_VARS="CONFIG=${CONFIG},NUM_REPLICATES=${NUM_REPLICATES},PERCENTILE=${PERCENTILE},ISO_THRESHOLD=${ISO_THRESHOLD}"
    ...
    if [ -n "$PREP_JOB_ID" ] && [ "$PREP_JOB_ID" != "XXXXX" ]; then
        ...
        RUN_CMD="sbatch --array=1-${ARRAY_SIZE} --dependency=afterok:${PREP_JOB_ID} \
            --export=${EXPORT_VARS} \
            --output=${LOG_DIR}/run_polyphest_${CONFIG}_p${PERCENTILE}_%A_%a.out \
            --error=${LOG_DIR}/run_polyphest_${CONFIG}_p${PERCENTILE}_%A_%a.err \
            ${RUN_SCRIPT}"
    else
    ...
    if [ "$DRY_RUN" = true ]; then
        ...
        echo "  Polyphest run job submitted: ${RUN_JOB_ID}"
    fi
    echo ""
fi
...
echo ""
echo "Check logs in:"
echo "  /groups/itay_mayrose/tomulanovski/gene2net/simulations/logs/"
echo "============================================================================"