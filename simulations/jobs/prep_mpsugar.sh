#!/bin/bash
#SBATCH --job-name=prep_mpsugar
#SBATCH --array=1-21
#SBATCH --output=/groups/itay_mayrose/tomulanovski/gene2net/simulations/logs/prep_mpsugar_%A_%a.out
#SBATCH --error=/groups/itay_mayrose/tomulanovski/gene2net/simulations/logs/prep_mpsugar_%A_%a.err
#SBATCH --time=2:00:00
#SBATCH --mem=4g
...
# PREP_MPSUGAR.SH
# ============================================================================
# Prepares SimPhy output for MP-SUGAR by:
#   1. Collecting gene trees from all batches (if applicable) into temp dir
#   2. Running Python script to:
#      - Strip taxon name suffixes
#      - Add copy numbers (taxon_1, taxon_2, ...)
#      - Output NEXUS format trees
#      - Output taxon map JSON
#
# Usage:
#   sbatch --export=CONFIG=conf_ils_low_10M prep_mpsugar.sh
#   sbatch --export=CONFIG=conf_ils_low_10M,NUM_REPLICATES=3 prep_mpsugar.sh
#
...
PYTHON_SCRIPT="/groups/itay_mayrose/tomulanovski/gene2net/simulations/scripts/process_for_mpsugar.py"
...
OUTPUT_BASE="${BASE_DIR}/${network}/processed/${CONFIG}/mpsugar_input"

echo "============================================================================"
echo "PREP MP-SUGAR - ${network}"
echo "============================================================================"
...
for replicate in $(seq 1 $NUM_REPLICATES); do
    ...
    echo "  Step 2: Processing trees for MP-SUGAR..."
    
    output_file="${output_dir}/mpsugar_trees.nex"
    map_file="${output_dir}/taxon_map.json"
    
    python "$PYTHON_SCRIPT" -i "$temp_dir" -o "$output_file" -m "$map_file"
...
echo "  - mpsugar_trees.nex  (NEXUS format gene trees)"
echo "  - taxon_map.json     (taxon mapping for MP-SUGAR)"
echo ""
