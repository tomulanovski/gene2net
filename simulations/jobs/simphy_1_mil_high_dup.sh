...
#SBATCH --array=1-21
#SBATCH --output=/groups/itay_mayrose/tomulanovski/gene2net/simulations/logs/simphy_height_1_million_trees_100000_ne_high_dup_job_%a.out
#SBATCH --error=/groups/itay_mayrose/tomulanovski/gene2net/simulations/logs/simphy_height_1_million_trees_100000_ne_high_dup_job_%a.err
#SBATCH --time=1:00:00
#SBATCH --mem=4g
...
echo "Running SimPhy for: ${network} - high dup (height 1 million)"

# Define paths - need to change according to run
SPECIES_TREE="${BASE_DIR}/${network}/species_tree_1_million.nex"
OUTPUT_DIR="${BASE_DIR}/${network}/data/high_dup_ne_100000_height_1_million_trees"
...
# Run SimPhy with specified duplication and loss rates (1e-8 per generation)
