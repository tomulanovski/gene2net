#!/bin/bash
#SBATCH --job-name=run_grandma_split_prior_real
#SBATCH --array=1-105
#SBATCH --output=/groups/itay_mayrose/tomulanovski/gene2net/simulations/logs/run_grandma_split_prior_real_%a.out
#SBATCH --error=/groups/itay_mayrose/tomulanovski/gene2net/simulations/logs/run_grandma_split_prior_real_%a.err
...
# ============================================================================
# RUN_GRANDMA_SPLIT_PRIOR_REAL.SH
# ============================================================================
# Runs GRANDMA_SPLIT with the REAL (ground-truth) ploidy prior (-x).
# Same gene trees / species tree as run_grandma_split_prior.sh (from
# grampa_input), but the ploidy prior is derived from the network's MUL-tree
# (simulations/networks/<network>.tre) instead of the inferred Polyphest
# multiset. Each species appears in the MUL-tree exactly its true-copy-number
# times, so make_real_multiset.py yields the true ploidy.
#
# The real multiset is renamed via the per-replicate taxa_map.txt (same as
# run_grandma_split_prior.sh) so names match the substring-fixed gene tree
# labels. Output goes to a separate grandma_split_prior_real/ dir so the
# inferred-prior results are left intact.
#
# Usage:
#   sbatch --export=CONFIG=conf_dup_loss_low_10M run_grandma_split_prior_real.sh
#
...
OUTPUT_DIR="${BASE_DIR}/${network}/results/${CONFIG}/grandma_split_prior_real/replicate_${replicate}"
...
MULTISET="${GRAMPA_INPUT_DIR}/multi_set_real.txt"
PLOIDY_FILE="${GRAMPA_INPUT_DIR}/split_ploidies_real.txt"
...
echo "============================================================================"
echo "RUN GRANDMA_SPLIT WITH REAL PLOIDY PRIOR"
echo "============================================================================"
...
# ============================================================================
# GENERATE REAL MULTISET FROM MUL-TREE, THEN PLOIDY FILE
# ============================================================================
...
echo "Generating ploidy file from real multiset..."
if [ -f "$TAXA_MAP" ]; then
...
# ============================================================================
# RUN GRANDMA_SPLIT WITH REAL PLOIDY PRIOR
# ============================================================================

echo "Starting GRANDMA_SPLIT with real ploidy prior..."
echo "============================================================================"
echo ""
...
if [ $exit_code -eq 0 ]; then
    echo "✓ GRANDMA_SPLIT (REAL PRIOR) COMPLETED SUCCESSFULLY"
    echo ""
    ...
    echo "✗ GRANDMA_SPLIT (REAL PRIOR) FAILED"
    echo ""
    echo "Network:   ${network}"
    echo "Replicate: ${replicate}"
