#!/usr/bin/env python3
"""
Script to generate taxa mapping for MPAllopp.
Fixed to correctly handle species names containing periods.

path: python "/groups/itay_mayrose/tomulanovski/gene2net/scripts/create_taxa_map.py" -i input_trees.nex -o taxamap.txt
"""
...
def generate_taxa_mapping(taxa_dict):
    """Generate the taxa mapping string for MPAllopp."""
    if not taxa_dict:
        print("Error: No taxa found to create mapping")
        return "<ERROR_EMPTY_MAPPING>"
    
    mapping = '<'
    for species, copies in sorted(taxa_dict.items()):
        # Ensure no empty copies
        copies = [c for c in copies if c]
        if not copies:
            continue
            
        mapping += f"{species}:{','.join(sorted(copies))}; "
    mapping = mapping.rstrip('; ') + '>'
    
    return mapping

def main():
    parser = argparse.ArgumentParser(description='Generate taxa mapping for MPAllopp from NEXUS or Newick files')
    parser.add_argument('-i', '--input', required=True, nargs='+', help='Input files containing trees')
    parser.add_argument('-o', '--output', required=True, help='Output file for the taxa mapping')
    ...
    mapping = generate_taxa_mapping(taxa_dict)
    
    # Write to output file
    try:
        with open(args.output, 'w') as f:
            f.write(mapping + '\n')
        print(f"Taxa mapping successfully written to {args.output}")
    except Exception as e:
