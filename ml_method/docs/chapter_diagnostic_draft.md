...
The model's error of 0.2017 decomposes into three parts. The build residual is 0.0474, the ASTRAL
backbone adds 0.1291 on top of that to reach the ceiling of 0.1765, and the model's own event
prediction adds a further 0.0252 to reach 0.2017. So the backbone is 0.1291 of the total, the build
residual is 0.0474, and event prediction is only 0.0252.
...
zero, so more than half of the validation networks reconstruct perfectly. The mean floor of 0.047
...
model is 0.025, while the gap from the floor to the ceiling is 0.129, so the backbone contributes
...
prediction can recover at most 0.025 of mu-distance, whereas the backbone accounts for 0.129. This
...
networks. Improving the event head cannot, because it is already within 0.025 of the ceiling.
...
the overall 0.047, and the honest reading of it is that the build is exact for the typical network
