...
class Gene2NetSample:
    ...
    @classmethod
    def load(cls, directory, clade_labels=False, away_labels=False):
        ...
        away_labels: if True, load the away-parent labels from labels_away.pkl
        (the clade labels with each allo partner retargeted to the non-home
        parent). Takes precedence over clade_labels. Raises FileNotFoundError if
        a requested sidecar is absent — never silently falls back.
        """
        ...
        sidecar = None
        if away_labels:
            sidecar = os.path.join(directory, "labels_away.pkl")
        elif clade_labels:
            sidecar = os.path.join(directory, "labels_clade.pkl")
        if sidecar is not None:
            if not os.path.exists(sidecar):
                raise FileNotFoundError(
                    f"{os.path.basename(sidecar)} requested but missing in {directory} — "
                    "run relabel_from_metadata.py first"
                )
...
class Gene2NetDataset:
    ...
    def __init__(self, root_dir, clade_labels=False, away_labels=False):
